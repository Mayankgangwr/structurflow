...
const VerifyEmailPendingFormContent: React.FC = () => {
    ...
    const [resolvedEmail, setResolvedEmail] = useState<string>("your email");
    ...
    const {
        ...
        timeLeft,
        maxReached
    } = useResendOTP(resolvedEmail);

    // Auto-submit if the user arrived with an otp in the url
    useEffect(() => {
        if (urlOtp && resolvedEmail && resolvedEmail !== "your email") {
            handleVerify({ otp: urlOtp }, resolvedEmail);
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [urlOtp, resolvedEmail]);
    ...
    const onSubmit = (data: any) => {
        handleVerify(data, resolvedEmail !== "your email" ? resolvedEmail : undefined);
    };
        ...
        <>
            ...
            <div className="mb-xl text-center">
                ...
                <p className="text-[15px] leading-5 font-normal text-on-surface-variant">
                    We&apos;ve sent a code to <span className="text-[13px] leading-[16px] tracking-[0.01em] font-semibold text-brand-primary">{resolvedEmail}</span>. Check your inbox to continue.
                </p>
            </div>
            ...
            <div className="flex flex-col space-y-sm pt-4 w-full">
                {maxReached ? (
                    ...
                    <button
                        ...
                        onClick={() => handleResend(resolvedEmail !== "your email" ? resolvedEmail : undefined)}
                    >
                        {isResending ? (
