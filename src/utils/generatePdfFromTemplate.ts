...
export async function generatePdfFromTemplate(
    templateElements: Array<ExtractedElement | PdfTextElement> | any,
    ...
    const elementsList: Array<any> = Array.isArray(templateElements)
        ? templateElements
        : (templateElements as any)?.elements || (templateElements as any)?.texts || [];
    ...
    if (options.templatePdfBuffer) {
        ...
        const textElements = elementsList.filter(
            (el) => !("type" in el) || el.type === "text"
        ) as PdfTextElement[];

        const rectangles = elementsList.filter(
        ...
        for (const [pageNum, pageTexts] of pageMap.entries()) {
            ...
            const pageVerticalLines = elementsList
                .filter(
                    (el) =>
                        "type" in el &&
                        el.type === "line" &&
