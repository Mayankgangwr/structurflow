...
const MobileBottomNav: React.FC = () => {
    ...
    const fullName = user?.name || `${user?.firstName || ""} ${user?.lastName || ""}`.trim() || "Workspace User";
                    ...
                    <div className="fixed bottom-0 left-0 right-0 max-h-[85vh] bg-white rounded-t-3xl shadow-2xl flex flex-col z-50 overflow-hidden border-t border-slate-200 animate-slide-up pb-safe">
                        ...
                        <div className="p-4 space-y-1.5 overflow-y-auto">
                            ...
                            <button
                                ...
                                className="w-full flex items-center justify-between p-3 rounded-xl transition-all hover:bg-red-50 text-red-600 cursor-pointer group"
                            >
                                <div className="flex items-center gap-3">
                                    <div className="w-9 h-9 rounded-lg bg-red-500/10 text-red-600 flex items-center justify-center group-hover:bg-red-500/20 transition-colors">
                                        <LogOut className="w-5 h-5" />
                                    </div>
                                    <div className="text-left">
                                        <p className="text-xs font-bold leading-none text-red-600">Log Out</p>
                                        <p className="text-[11px] text-red-400 mt-0.5">Sign out of your account</p>
                                    </div>
                                </div>
                        ...
                        {/* User Profile Card at Bottom of Drawer (Clickable to trigger Logout) */}
                        <div className="p-4 border-t border-slate-100 bg-slate-50/70 flex items-center justify-between gap-3">
                            ...
                            >
                                <div className="w-9 h-9 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-xs border border-primary/20 shrink-0 overflow-hidden">
                                    {user?.image ? (
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-1.5">
                                        <p className="text-xs font-bold text-slate-900 truncate group-hover:text-primary transition-colors">{fullName}</p>
                                        {displayRole && (
                                            <span className="text-[9px] font-bold px-1.5 py-0.2 bg-slate-200 text-slate-700 rounded uppercase shrink-0">
                                                {displayRole}
                                            </span>
                                        )}
                                    </div>
