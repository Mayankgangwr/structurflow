    ...
    return (
        <div className="w-full flex flex-col">
            <div className="w-full flex justify-between items-center gap-3">
                <h1
                    className="font-headline-md-mobile md:font-headline-lg text-headline-md-mobile md:text-headline-lg text-text-primary">
                    {name || `Project ${id}`}
                </h1>
                <Badge
                    className={cn("px-2.5 py-1 text-sm font-medium flex items-center gap-1.5 border",
                        status === "Inactive" ? "bg-warning/10 text-warning  border-warning/20" :
                            "bg-green-200 text-green-700 border-green-400"
                    )}
                >
                    {status !== "Inactive" ? <CheckCircle2 className="w-4 h-4" /> : <CircleDashed className="w-4 h-4 text-secondary" />}
                    {status}
                </Badge>
            </div>
